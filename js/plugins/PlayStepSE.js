//=============================================================================
// PlayStepSE.js (original configuration)
//=============================================================================
// Release History
// 2016.Aug.02  Ver1.0.0  First Release
// 2018.Oct.01  (original) add option to play region se
//  see https://forum.tkool.jp/index.php?threads/63/
 ...
 * <StepRegion5SE:Noise,20,150,3>
 *
 * ・★リージョンのオリジナル仕様(本バージョン限定)
 * マップ、イベント共通：
 * <StepSE2:filename,volume,pitch,interval> イベントのデフォルトの足音を設定
 * <StepRegion<regionId>SE2:filename,volume,pitch,interval> マップ内の
 *  指定したリージョンでの足音を設定
 * ★上記で足音の指定がある場合、StepSEなどで指定した足音と交互に、
 *  足音を演奏する。
 * 
 * [プラグインコマンド]
 * ・デフォルト足音の一時的変更
...
(function() {
  ...
  var processNote = function(meta) {
    ...
    if (this.stepSeInMemo) {
      someoneSet = true;
      // se of 2nd step
      this.stepSeInMemo2 = noteValueToSe(meta.StepSE2);
    }
    // region setting
    var se;
    for (var i = 1; i <= 255; i++) {
      ...
      if (se) {
        ...
        this.stepRegionSeInMemo2 = this.stepRegionSeInMemo2 || [];
        someoneSet = true;
        // se of 2nd step
        se = noteValueToSe(meta['StepRegion' + i + 'SE2']);
        if (se) {
          this.stepRegionSeInMemo2[i] = se;
        }
      }
    }
    return someoneSet;
  ...
  Game_CharacterBase.prototype.stepSeInMemo2R = function (se) {
    if ($gameMap.stepSeInMemo2 && (this.stepCount2++ % 2)) {
      return $gameMap.stepSeInMemo2;
    }
    return se;
  };

  Game_CharacterBase.prototype.stepRegionSeInMemo2 = function (se, regionId) {
    if ($gameMap.stepRegionSeInMemo2[regionId] && (this.stepCount2++ % 2)) {
      return $gameMap.stepRegionSeInMemo2[regionId];
    }
    return se;
  };
  ...
  Game_Player.prototype.stepSound = function(regionId) {
    ...
    if ((se = this.stepRegionSeByCommand) && se[regionId]) {
      ...
      return this.stepRegionSeInMemo2(se[regionId], regionId);
    } else if (se = this.stepSeByCommand) {
      return se;
    } else if (se = $gameMap.stepSeInMemo) {
      return this.stepSeInMemo2R(se);
